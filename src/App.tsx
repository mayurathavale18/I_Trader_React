...
function App() {
  ...
  const { states, trainModel, backtestModel, startTrading, forceExit } = useStockBot();
  
  // Check if any action is currently loading
  const isLoading = states.train.loading || states.backtest.loading || states.trade.loading || states.forceExit.loading;
  ...
  const getStatusMessage = () => {
    ...
    return selectedStock ? `Selected: ${selectedStock}` : "Please select a stock";
  };
    ...
    <div className={`min-h-screen ${
      ...
      <main className="container mx-auto px-4 py-10">
        ...
        <section className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold mb-4">Algorithmic Trading Bot</h2>
          ...
          <Card className={`lg:col-span-4 ${
            ...
            <div className="p-2">
              ...
              <div className="space-y-6 p-2">
                <div>
                  <label htmlFor="stock-select" className={`block text-sm font-medium mb-2 ${
                    isDarkMode ? "text-blue-300" : "text-blue-600"
                  }`}>
                ...
                <div className="space-y-3 pt-4">
                  ...
                  >
                    {states.forceExit.loading ? (
                      ...
                      <>
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                          <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM7 10a1 1 0 011-1h1a1 1 0 110 2H9v3a1 1 0 11-2 0v-3z" clipRule="evenodd" />
                        </svg>
                        Force Exit
                      </>
                    )}
              ...
              <div className="p-6">
                ...
                {selectedStock ? (
                    ...
                    <div className="flex justify-between items-center mb-4 p-2">
                      <div>
                        <h4 className="font-bold text-lg">
                          {stockOptions.find(s => s.value === selectedStock)?.label || selectedStock}
                        </h4>
                        <p className={`text-sm ${
                          isDarkMode ? "text-blue-300" : "text-blue-600"
                        }`}>Real-time trading data</p>
                      </div>
                      <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full mr-2 ${
                          isLoading ? 'bg-yellow-400' :
                          states.trade.message && !states.trade.error ? 'bg-green-400 animate-pulse' : 
                          isDarkMode ? 'bg-slate-400' : 'bg-gray-400'
                        }`}></div>
                        <p className="text-sm font-medium">
                          {isLoading ? 'Processing' : 
                           states.trade.message && !states.trade.error ? 'Trading' : 'Idle'}
                        </p>
                      </div>
                    </div>
                    <div className={`h-64 ${
                      isDarkMode ? "bg-slate-800" : "bg-white border-gray-300 border"
                      ...
                      <div className="text-center p-4">
                        ...
                        <p className={isDarkMode ? "text-slate-400" : "text-gray-500"}>
                          Stock performance chart will appear here
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className={`h-64 flex items-center justify-center ${
                    ...
                    <div className="text-center p-4">
                      <svg xmlns="http://www.w3.org/2000/svg" className={`h-12 w-12 mx-auto ${
                        isDarkMode ? "text-slate-600" : "text-gray-400"
                      } mb-2`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className={`${
                    isDarkMode
                      ? "bg-slate-900 border-slate-700" 
                      : "bg-gray-100 border-gray-300"
                    } rounded-lg border ${states.train.error ? 'border-red-500' : ''} p-4`}>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className={`text-sm font-medium ${
                        isDarkMode ? "text-blue-300" : "text-blue-600"
                      }`}>Model Training</h4>
                      <div className={`w-3 h-3 rounded-full ${
                        states.train.loading ? 'bg-yellow-400' :
                        states.train.error ? 'bg-red-500' :
                        states.train.message ? 'bg-green-500' : 
                        isDarkMode ? 'bg-slate-500' : 'bg-gray-400'
                      }`}></div>
                    </div>
                    <p className="text-sm truncate">
                      {states.train.loading ? 'Training in progress...' :
                       states.train.message ? states.train.message : 'Ready to train'}
                    </p>
                  </div>
                  ...
                  <div className={`${
                    isDarkMode
                      ? "bg-slate-900 border-slate-700" 
                      : "bg-gray-100 border-gray-300"
                    } rounded-lg border ${states.backtest.error ? 'border-red-500' : ''} p-4`}>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className={`text-sm font-medium ${
                        isDarkMode ? "text-blue-300" : "text-blue-600"
                      }`}>Backtesting</h4>
                      <div className={`w-3 h-3 rounded-full ${
                        states.backtest.loading ? 'bg-yellow-400' :
                        states.backtest.error ? 'bg-red-500' :
                        states.backtest.message ? 'bg-green-500' : 
                        isDarkMode ? 'bg-slate-500' : 'bg-gray-400'
                      }`}></div>
                    </div>
                    <p className="text-sm truncate">
                      {states.backtest.loading ? 'Backtesting in progress...' :
                       states.backtest.message ? states.backtest.message : 'Ready to backtest'}
                    </p>
                  </div>
                  ...
                  <div className={`${
                    isDarkMode
                      ? "bg-slate-900 border-slate-700" 
                      : "bg-gray-100 border-gray-300"
                    } rounded-lg border ${states.trade.error ? 'border-red-500' : ''} p-4`}>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className={`text-sm font-medium ${
                        isDarkMode ? "text-blue-300" : "text-blue-600"
                      }`}>Trading Status</h4>
                      <div className={`w-3 h-3 rounded-full ${
                        states.trade.loading ? 'bg-yellow-400' :
                        states.trade.error ? 'bg-red-500' :
                        states.trade.message ? 'bg-green-500 animate-pulse' : 
                        isDarkMode ? 'bg-slate-500' : 'bg-gray-400'
                      }`}></div>
                    </div>
                    <p className="text-sm truncate">
                      {states.trade.loading ? 'Processing trading request...' :
                       states.trade.message ? states.trade.message : 'Ready to trade'}
                    </p>
                  </div>
                  ...
                  <div className={`${
                    isDarkMode
                      ? "bg-slate-900 border-slate-700" 
                      : "bg-gray-100 border-gray-300"
                    } rounded-lg border ${states.forceExit.error ? 'border-red-500' : ''} p-4`}>
                    <div className="flex items-center justify-between mb-2">
                      <h4 className={`text-sm font-medium ${
                        isDarkMode ? "text-blue-300" : "text-blue-600"
                      }`}>Force Exit</h4>
                      <div className={`w-3 h-3 rounded-full ${
                        states.forceExit.loading ? 'bg-yellow-400' :
                        states.forceExit.error ? 'bg-red-500' :
                        states.forceExit.message ? 'bg-green-500' : 
                        isDarkMode ? 'bg-slate-500' : 'bg-gray-400'
                      }`}></div>
                    </div>
                    <p className="text-sm truncate">
                      {states.forceExit.loading ? 'Processing exit request...' :
                       states.forceExit.message ? states.forceExit.message : 'Ready for emergency exit'}
                    </p>
                  </div>
                </div>
